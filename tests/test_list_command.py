...
SUMMARIES = [
    DeviceSummary("SW", "M", "usw", "aa", "1.2.3.4"),
    DeviceSummary("GW", "UDMPRO", "udm", "bb", "192.168.1.1", wan_ip="203.0.113.7"),
]
...
def _wire(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(list_cmd, "list_devices", lambda client: SUMMARIES)
...
def test_default_table_omits_the_wan_column(capsys: pytest.CaptureFixture[str]) -> None:
    list_cmd.devices()
    out = capsys.readouterr().out
    assert "WAN" not in out
    assert "203.0.113.7" not in out


def test_wan_flag_adds_a_wan_column(capsys: pytest.CaptureFixture[str]) -> None:
    list_cmd.devices(wan=True)
    out = capsys.readouterr().out
    assert "WAN" in out
    assert "203.0.113.7" in _line(out, "GW")


def test_wan_column_shows_a_placeholder_for_devices_with_no_wan_address(
    capsys: pytest.CaptureFixture[str],
) -> None:
    list_cmd.devices(wan=True)
    assert _line(capsys.readouterr().out, "SW").rstrip().endswith("-")
